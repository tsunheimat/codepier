...
window.CodePierIdentity=(()=>{
  ...
  function bind(){
    ...
    $$('[data-iam-check]').forEach(x=>x.onclick=()=>busy(x,async()=>{const out=await post('/api/iam/oidc/providers/'+x.dataset.iamCheck+'/check');modal('OIDC 发现已验证',`<p>签名密钥：${out.signing_keys}</p><p>精确回调：</p><div class="code-box iam-wrap">${esc(out.callback)}</div><p>Back-channel logout：</p><div class="code-box iam-wrap">${esc(out.backchannel_logout)}</div>`);}));
