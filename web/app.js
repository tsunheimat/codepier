...
const nav=[['native','terminal','CLI 会话','11'],['overview','dashboard','控制总览','01'],['devices','device','设备节点','02'],['projects','folder','项目映射','03'],['vps','cloud','VPS 管理','13'],['workbench','code','远程工作台','04'],['workflows','history','开发任务','05'],['audit','audit','操作审计','06'],['connect','plug','MCP 接入','07'],['profiles','shield','访问 Profiles','14'],['roles','shield','访问角色','15'],['diagnostics','activity','运行诊断','08'],['integrations','network','开发工具','12'],['artifacts','download','产物交付','09'],['identity','key','我的账号','16'],['members','shield','空间成员','17'],['identity-admin','shield','身份管理','18'],['settings','settings','系统设置','10']];
const S={space_id:null,identity:null,session:null,page:'overview',overview:null,projects:[],devices:[],settings:null,events:null,renderSeq:0,auditMode:'operations',auditOffset:0,auditSource:'',auditStatus:'',auditQuery:'',auditNext:null,work:{project:'',path:'',sha:'',original:'',content:'',dirty:false,truncated:false,treePath:'.',treeOffset:0,treeNext:null,tree:[],task:'',operation:null,console:''},poll:null,modalCleanup:null};
...
function sessionOwner(session){return session?.user_id?'id:'+session.user_id+':space:'+(S.space_id||'legacy'):session?.username?'name:'+session.username:null;}
...
function endSession(discard=false){
  ...
  window.CodePierIdentity?.reset();S.identity=null;S.space_id=null;
  renderLogin();
}
async function api(path,options={}){
  const {retrySafe=false,retryDelays=[500,1000,2000,4000],requestTimeout=15000,signal:externalSignal,...request}=options;
  const session=S.session,space=S.space_id,method=(request.method||'GET').toUpperCase();
  const safe=method==='GET'||retrySafe;
  const headers={...(space?{'X-CodePier-Space':space}:{}),...(request.body?{'Content-Type':'application/json'}:{}),...(session?.csrf?{'X-RD-CSRF':session.csrf}:{}),...request.headers};
...
function renderLogin(configured=true){
  ...
  uiLabelFields($('#login-form'));CodePierIdentity.loginButtons();
...
function renderShell(){
  ...
  const groups=[['工作区',['overview','devices','projects','vps']],['开发',['native','workbench','integrations','workflows','artifacts','audit']],['系统',['connect','profiles','roles','identity','members','identity-admin','diagnostics','settings']]];
...
async function bootAuthenticated(){
  try {
    ...
    stopEvents();if(typeof stopComputerApprovals==='function')stopComputerApprovals();
    if(!S.session)return;
    CodePierIdentity.reset();S.identity=null;S.space_id=null;
    $('#app').innerHTML=`<main class="boot"><div id="page" role="alert">${brand}${notice(esc(error.message))}<button class="btn primary" data-action="retry-identity">重试身份与空间校验</button><button class="btn ghost" data-action="logout">退出登录</button></div></main>`;
    return;
  }
...
function connectEvents(){
  stopEvents();const session=S.session,events=S.events=new EventSource('/api/events?'+new URLSearchParams({space_id:S.space_id||''}));
  ...
  events.onmessage=e=>{
    ...
    if(m.type==='iam'){CodePierIdentity.refresh().catch(error=>toast(error.message,true));}
...
async function renderPage(showLoading=true){window.CodePierPanelUpdate?.detach();window.CodePierIntegrations?.detach();if(!S.session||!$('#page'))return;if(S.page==='native'){const seq=++S.renderSeq;try{await chatPage();}catch(e){if(S.session&&S.page==='native'&&seq===S.renderSeq){chatDetach();$('#page').innerHTML=notice(esc(e.message))+`<button class="btn" data-action="refresh">重试</button>`;}}return;}if(typeof chatDetach==='function')chatDetach();const seq=++S.renderSeq;const page=S.page;if(showLoading)$('#page').innerHTML='<div class="skeleton" role="status" aria-label="正在加载页面"></div>';try{let html='';if(['identity','members','identity-admin'].includes(page)){html=await CodePierIdentity.html(page);}else if(page==='overview'){S.overview=await api('/api/overview');S.projects=S.overview.projects;S.devices=S.overview.devices;html=overviewHTML();}else if(page==='devices'){await loadBasics();html=devicesHTML();}else if(page==='projects'){await loadBasics();html=projectsHTML();}else if(page==='vps'){html=await vpsHTML(seq);}else if(page==='workbench'){await loadBasics();if(!S.work.project&&S.projects.length)S.work.project=S.projects[0].id;html=workbenchHTML();}else if(page==='workflows'){await loadBasics();html=await workflowsHTML(seq);}else if(page==='audit'){html=await auditHTML(seq);}else if(page==='roles'){await loadBasics();html=await CodePierRoles.html();}else if(page==='profiles'){await loadBasics();html=await CodePierProfiles.html();}else if(page==='connect'){S.settings=await api('/api/settings');await loadBasics();S.grants=(await api('/api/grants')).grants;html=connectHTML();}else if(page==='integrations'){await loadBasics();html=CodePierIntegrations.html();}else if(page==='diagnostics'){await loadBasics();html=await diagnosticsHTML(seq);}else if(page==='artifacts'){await loadBasics();html=await artifactsHTML(seq);}else if(page==='settings'){S.settings=await api('/api/settings');html=settingsHTML();}if(!showLoading)await uiWaitForPagePointer();if(seq!==S.renderSeq||!S.session)return;const presentation=showLoading?null:uiCapturePage();if(page==='vps'&&!showLoading)vpsReplacePage(html);else $('#page').innerHTML=html;uiPageReady(showLoading,presentation);if(['identity','members','identity-admin'].includes(page))CodePierIdentity.bind();if(page==='roles')CodePierRoles.bind();if(page==='profiles')CodePierProfiles.bind();if(page==='vps')bindVps();if(page==='workbench')bindWorkbench();if(page==='workflows')bindWorkflows();if(page==='audit')bindAudit();if(page==='integrations')CodePierIntegrations.bind();if(page==='diagnostics')bindDiagnostics();if(page==='artifacts')bindArtifacts();if(page==='settings')bindSettings();CodePierIdentity.applyCapabilities();}catch(e){if(seq===S.renderSeq&&S.session)$('#page').innerHTML=notice(esc(e.message))+`<div class="spacer"></div><button class="btn" data-action="refresh">重试</button>`;}}
